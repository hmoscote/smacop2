...
namespace smacop2
{
        ...
        public class op_formatos
        {
            ...
            public op_formatos(string path)
            {
                ruta = @path;
            }
            ...
            public void Export(ArrayList titulos, DataTable datos)
            {
                try
                {
                    ...
                    html.Append(@"<title>SMACOP"+DateTime.Now.ToFileTime().ToString()+"</title>");
                    html.Append(@"</head>");
                    html.Append(@"<body>");

                    //Generando encabezados del archivo
                    html.Append(@"<table WIDTH=730 CELLSPACING=0 CELLPADDING=10 border=4 BORDERCOLOR=" + comillas + "#333366" + comillas + " bgcolor=" + comillas + "#FFFFFF" + comillas + ">");
                    html.Append(@"<tr> <b>");
